...
export default function Home() {
  ...
  const getTeamColor = (teamName: string) => {
    switch(teamName) {
      ...
      case 'fuoco': return 'text-red-200';
      case 'erba': return 'text-green-200';
  ...
  const getTeamBadge = (teamName: string) => {
    switch(teamName) {
      ...
      case 'fuoco': return 'bg-red-800 text-red-100';
      case 'erba': return 'bg-green-800 text-green-100';
  ...
  return (
    <div className={`container mx-auto px-4 py-8 mt-16 ${styles.textColor}`}>
        ...
        {sortedTeams.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {sortedTeams.map((team, index) => (
              <div key={team} className={`${styles.cardBg} border ${styles.border} rounded-lg p-6 shadow-md relative overflow-hidden`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getTeamBadge(team)} mb-2`}>
                ...
                <div className="mt-4">
                  <Link href="/classifica" className={`text-sm font-medium ${styles.link}`}>
          ...
          <div className={`${styles.cardBg} border ${styles.border} rounded-lg p-6 shadow-md mb-12 text-center`}>
            <p className={styles.textMuted}>Dati del team in caricamento o non disponibili.</p>
          </div>
        )}
      </section>
          ...
          {featuredEvents.map(event => (
            <div key={event.id} className={`${styles.cardBg} border ${styles.border} rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow`}>
              ...
              <div className="p-6">
                ...
                <p className={`${styles.textMuted} mb-4 line-clamp-2`}>{event.description}</p>
      ...
      <section className="mb-16">
        ...
        <div className={`${styles.cardBg} rounded-lg shadow-md overflow-hidden border ${styles.border}`}>
          ...
          <div className="p-4">
            {topUsers.length > 0 ? (
              topUsers.map((user, i) => (
                <div key={user.id} className={`flex items-center justify-between py-2 border-b last:border-0 border-gray-700`}>
                  <div className="flex items-center">
                    <span className={`w-6 text-center font-medium mr-4 ${i === 0 ? 'text-amber-500' : ''}`}>{i + 1}</span>
      ...
      {/* Club */}
      <section className={`px-4 py-16 relative overflow-hidden ${styles.mainGradient}`}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className={`rounded-lg overflow-hidden shadow-lg transition-transform hover:scale-105 ${styles.cardBg} border ${styles.border}`}>
              ...
              <div className="p-6">
                <h3 className={`text-xl font-bold mb-2 ${styles.textColor}`}>Spaghettistudio</h3>
                <p className={`mb-4 ${styles.textMuted}`}>Sviluppa videogiochi e applica le tue competenze di programmazione nel mondo del gaming.</p>
                <Link href="/club/gaming" className={`inline-block px-4 py-2 rounded-lg ${styles.button}`}>
                  Scopri di più
                </Link>
              </div>
            </div>
            ...
            <div className={`rounded-lg overflow-hidden shadow-lg transition-transform hover:scale-105 ${styles.cardBg} border ${styles.border}`}>
              ...
              <div className="p-6">
                <h3 className={`text-xl font-bold mb-2 ${styles.textColor}`}>Club Libro e Manga</h3>
                <p className={`mb-4 ${styles.textMuted}`}>Esplora il mondo della letteratura e dei manga con altri appassionati di lettura.</p>
                <Link href="/club/libri" className={`inline-block px-4 py-2 rounded-lg ${styles.button}`}>
                  Scopri di più
                </Link>
              </div>
            </div>
            ...
            <div className={`rounded-lg overflow-hidden shadow-lg transition-transform hover:scale-105 ${styles.cardBg} border ${styles.border}`}>
              ...
              <div className="p-6">
                <h3 className={`text-xl font-bold mb-2 ${styles.textColor}`}>Club Cybersecurity</h3>
                <p className={`mb-4 ${styles.textMuted}`}>Impara le tecniche di sicurezza informatica e partecipa a competizioni CTF.</p>
                <Link href="/club/cybersecurity" className={`inline-block px-4 py-2 rounded-lg ${styles.button}`}>
                  Scopri di più
                </Link>
              </div>
            </div>
